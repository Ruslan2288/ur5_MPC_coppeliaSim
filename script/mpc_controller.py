...
class MPC_controller():
    def __init__(self, p, m, n, dt):
        ...
        #
        one = np.ones((1, n))        
        self.Gq = np.eye(p*n)*np.concatenate((one*np.full(n, 100), one*np.full(n, 40), one*np.full(n, 90), one*np.full(n, 50), one*np.full(n, 50)), axis=1)
        self.Gw = np.eye(p*n)*np.concatenate((one*np.full(n, 0.5), one*np.full(n, 0.4), one*np.full(n, 0.3), one*np.full(n, 0.2), one*np.full(n, 0.1)), axis=1)
